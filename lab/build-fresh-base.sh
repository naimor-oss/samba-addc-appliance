...
if [[ -n "$SSH_PUBKEY" ]]; then
    ...
    if [[ $have_keys -eq 0 ]]; then
        # Legacy single-developer fallback. Stager errors out here if
        # the file is missing, which is the right behavior — operator
        # gets a clear message instead of a key-less appliance.
